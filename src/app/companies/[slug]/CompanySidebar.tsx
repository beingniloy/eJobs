...
export default function CompanySidebar({ company, whyJoinUs, brochures, topSkills, socialLinks, location }: Props) {
  return (
    <div className="lg:col-span-3 space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          {company.website && <div className="flex items-center gap-2"><Globe className="h-4 w-4 text-muted-foreground" /><div><p className="text-xs text-muted-foreground">Website</p><a href={company.website.startsWith("http") ? company.website : `https://${company.website}`} target="_blank" rel="noopener noreferrer" className="font-medium text-primary hover:underline flex items-center gap-1">{company.website}<ExternalLink className="h-3 w-3" /></a></div></div>}
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-1.5">
              {topSkills.map((skill) => <Badge key={skill} variant="secondary" className="text-[10px]">{skill}</Badge>)}
            </div>
          </CardContent>
        </Card>
      )}
