      ...
      <Tabs value={tab} onValueChange={(v) => { setTab(v); if (v === "add") { setPersonalStep(1); } }}>
        ...
        <TabsContent value="withdraw"><Card><CardHeader><CardTitle>Withdraw</CardTitle></CardHeader><CardContent className="space-y-4">
          ...
          {selWG && <>
            ...
            {selWG.user_input && Array.isArray(selWG.user_input) && selWG.user_input.map((f: any, idx: number) => <div key={f.key || idx} className="space-y-2"><Label>{f.label}{f.required ? " *" : ""}</Label><Input placeholder={f.label} value={withdrawInputs[f.key] || ""} onChange={(e) => setWithdrawInputs({ ...withdrawInputs, [f.key]: e.target.value })} /></div>)}
