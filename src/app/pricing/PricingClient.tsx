...
export default function PricingClient() {
  ...
  const getUpgradeCredit = (activeSub: any) => {
    ...
    const oldPrice = Number(
      activeSub.plan_details?.price || activeSub.plan?.price || 0
    );
    ...
    const totalDays = Math.max(
      1,
      Math.round(
        (expires.getTime() - starts.getTime()) / (1000 * 60 * 60 * 24)
      )
    );
    let usedDays = Math.round(
      (now.getTime() - starts.getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  const getButtonState = (plan: Plan) => {
    ...
    if (!isAuthenticated) {
      return {
        label: isBn ? "এখনই শুরু করুন" : "Get Started",
        disabled: false,
    ...
    if (
      ...
      const currentPrice = Number(
        currentSubscription.plan_details?.price ||
          currentSubscription.plan.price ||
          0
      );
      if (plan.price < currentPrice) {
        return {
          label: isBn ? "ডাউনগ্রেড সম্ভব নয়" : "Downgrade Restricted",
          disabled: true,
          variant: "outline" as const,
  ...
  return (
    <PublicLayout>
      ...
      <PricingPurchaseModal
        ...
        upgradeCredit={
          currentSubscription ? getUpgradeCredit(currentSubscription) : 0
        }
        onSuccess={loadData}
      />
    </PublicLayout>
