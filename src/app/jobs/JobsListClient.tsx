...
import {
  ...
  MessageSquare, Banknote, Filter, Mail, Rocket, DollarSign, Loader2,
} from "lucide-react";
...
export default function JobsListClient() {
  ...
  const handleToggleSave = async (jobId: string | number) => {
    if (!isAuthenticated) {
      toast.error(isBn ? "আগে লগইন করুন" : "Please login first");
      return;
    }
  ...
  useEffect(() => {
    ...
    api.get("/jobs/stats").then((res) => setPlatformStats(res.data?.data)).catch(() => { /* handled */ });
    api.get("/companies/featured").then((res) => setFeaturedCompanies(res.data?.data || [])).catch(() => { /* handled */ });
  }, []);
    ...
    <PublicLayout>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[var(--left)_1fr_var(--right)] lg:gap-6"
          style={{ '--left': leftOpen ? '220px' : '36px', '--right': rightOpen ? '280px' : '36px' } as React.CSSProperties}>
          ...
          <aside className="space-y-5 min-w-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {leftOpen && <Filter className="h-4 w-4 text-primary" />}
                {leftOpen && <h3 className="font-semibold text-sm whitespace-nowrap">{isBn ? "ফিল্টার" : "Filter"}</h3>}
              </div>
              {leftOpen && <button onClick={() => { setSelectedJobType("all"); setSelectedExperience("All Levels"); setSelectedSalary(0); setBudgetMin(""); setBudgetMax(""); setCategory("all"); setCurrentPage(1); }} className="shrink-0 text-xs text-primary hover:underline cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">{isBn ? "সব মুছুন" : "Clear All"}</button>}
            </div>
            ...
            {leftOpen && (
              <>
                  ...
                  <div className="space-y-1.5">
                    {JOB_TYPES.map((t) => (
                      <label key={t.value} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary">
                        <input type="radio" name="jobType" checked={selectedJobType === t.value} onChange={() => { setSelectedJobType(t.value); setCurrentPage(1); }} className="accent-primary cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2" />
                        {t.label}
                      </label>
                    ))}
                  </div>
                ...
                {/* Experience */}
                <div>
                  <p className="text-xs font-semibold mb-2 uppercase tracking-wider text-muted-foreground">{isBn ? "অভিজ্ঞতা" : "Experience"}</p>
                  <div className="space-y-1.5">
                    {EXPERIENCE_LEVELS.map((l) => (
                      <label key={l} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary">
                        <input type="radio" name="experience" checked={selectedExperience === l} onChange={() => setSelectedExperience(l)} className="accent-primary cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2" />
                        {l}
                      </label>
                    ))}
                  </div>
                ...
                {/* Salary */}
                <div>
                  <p className="text-xs font-semibold mb-2 uppercase tracking-wider text-muted-foreground">{isBn ? "বেতন" : "Salary Range"}</p>
                  <div className="space-y-1.5">
                    {SALARY_RANGES.map((s, i) => (
                      <label key={i} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary">
                        <input type="radio" name="salary" checked={selectedSalary === i} onChange={() => handleSalarySelect(i)} className="accent-primary cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2" />
                        {s.label}
                      </label>
                    ))}
                  </div>
          ...
          <main className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                {isBn ? "মোট" : "Total"} <span className="font-semibold text-foreground">{totalJobs}</span> {isBn ? "টি চাকরি" : "jobs found"}
              </p>
            </div>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <Link href={`/jobs/${job.id}`} className="font-semibold text-base hover:text-primary transition-colors line-clamp-1">
                                    {job.title}
                                    {job.is_promoted ? <Rocket className="inline h-3.5 w-3.5 ml-1.5 text-amber-500" /> : null}
                                  </Link>
              ...
              <>
                ...
                <Card>
                  <CardContent className="p-5">
                    ...
                    <div className="space-y-3">
                      {featuredCompanies.length > 0 ? featuredCompanies.map((c) => (
                ...
                <Card className="border-primary/20">
                  <CardContent className="p-5 space-y-3">
                    ...
                    <div className="space-y-2 text-xs text-muted-foreground">
                      {[
                        { icon: ShieldCheck, text: isBn ? "১০০% যাচাইকৃত কোম্পানি" : "100% Verified Companies" },
                        { icon: Award, text: isBn ? "AI-পাওয়ার্ড ম্যাচিং" : "AI-Powered Matching" },
                        { icon: CheckCircle2, text: isBn ? "২৪/৭ সাপোর্ট" : "24/7 Support" },
                      ].map((item) => (
      ...
      <section className="border-t bg-muted/20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <h2 className="text-lg font-bold mb-1">{isBn ? "ক্যাটাগরি অনুযায়ী চাকরি" : "Browse Jobs by Category"}</h2>
          <p className="text-sm text-muted-foreground mb-6">{isBn ? "আপনার পছন্দের ক্যাটাগরি খুঁজুন" : "Find your dream job in the right category"}</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {categories.slice(0, 7).map((c) => (
              <Link key={c.value} href={`/jobs/category/${c.value}`} className="flex flex-col items-center gap-2 p-4 rounded-xl border bg-card hover:shadow-md hover:border-primary/30 transition-all text-center group">
                <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                  <Briefcase className="h-5 w-5 text-primary" />
                </div>
                <p className="text-xs font-medium leading-tight">{c.label}</p>
                <p className="text-[11px] text-muted-foreground">{c.count || 0} {isBn ? "জব" : "Jobs"}</p>
              </Link>
            ))}
          </div>
          <div className="text-center mt-4">
            <Button variant="outline" size="sm" asChild><Link href="/jobs">{isBn ? "সব ক্যাটাগরি দেখুন →" : "All Categories →"}</Link></Button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{isBn ? "ইমেইল দিন এবং নতুন চাকরি জানুন" : "Subscribe to get the latest job alerts"}</p>
            </div>
