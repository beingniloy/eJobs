...
export default function TemplateStep({ wizard, onPrev }: { wizard: ReturnType<typeof useResumeWizard>; onPrev: () => void }) {
  ...
  useEffect(() => {
    resumeService.getTemplates().then((t) => { setTemplates(t); setLoading(false); }).catch(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!selectedSlug) return;
    setPreviewLoading(true);
    resumeService.getPreviewDemo(selectedSlug).then((html) => { if (html) setPreviewHtml(html); }).catch(() => {}).finally(() => setPreviewLoading(false));
  }, [selectedSlug]);
  ...
  const handleCreateAndDownload = async () => {
    ...
    } catch (e: any) { toast.error(e?.response?.data?.message || "Failed to create CV"); }
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">{[...Array(8)].map((_, i) => <Skeleton key={i} className="h-56 rounded-xl" />)}</div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {templates.map((t, idx) => (
            <button key={t.id} onClick={() => { setSelectedSlug(t.slug); setSectionData("template_slug", t.slug); }}
              className={`text-left rounded-xl border-2 overflow-hidden transition-all hover:shadow-md ${selectedSlug === t.slug ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-primary/40"}`}>
              <div className={`h-36 bg-gradient-to-br ${TEMPLATE_GRADIENTS[idx % TEMPLATE_GRADIENTS.length]} flex items-center justify-center`}><FileText className="h-10 w-10 text-white/40" /></div>
              <div className="p-3">
          ...
          <div className="lg:w-72 space-y-4">
            <Card><CardContent className="p-4 space-y-3">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${TEMPLATE_GRADIENTS[templates.indexOf(selectedTemplate) % TEMPLATE_GRADIENTS.length]} flex items-center justify-center`}><FileText className="h-6 w-6 text-white/60" /></div>
              ...
              <Button variant="outline" className="w-full" onClick={onPrev}>Edit Details</Button>
            </CardContent></Card>
          </div>
        </div>
      )}
